...
export default function LivreBlancDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/resources/${slug}`)
      .then((res) => { if (res.status === 404) { router.push("/livresblancs"); return; } return res.json(); })
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-2 text-sm text-text-gray">
          ...
          <Link href="/livresblancs" className="hover:text-primary">Livres Blancs</Link>
          <ChevronRight className="w-3 h-3" />
      ...
      <article className="max-w-4xl mx-auto px-4 py-12">
        <Link href="/livresblancs" className="flex items-center gap-2 text-text-gray hover:text-primary text-sm mb-8">
          <ArrowLeft className="w-4 h-4" /> Retour aux livres blancs
        </Link>

        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium flex items-center gap-1">
              <BookOpen className="w-3 h-3" /> Livre Blanc
            </span>
          </div>
          ...
          {resource.excerpt && (
            <p className="text-lg text-text-gray leading-relaxed border-l-4 border-orange pl-4 italic mt-6">
              {resource.excerpt}
            </p>
          )}
        </motion.div>
