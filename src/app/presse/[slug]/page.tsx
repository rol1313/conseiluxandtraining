...
import { ArrowLeft, Newspaper, ChevronRight, ExternalLink, User } from "lucide-react";
...
export default function PresseDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/resources/${slug}`)
      .then((res) => { if (res.status === 404) { router.push("/presse"); return; } return res.json(); })
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-2 text-sm text-text-gray">
          ...
          <Link href="/presse" className="hover:text-primary">Presse</Link>
          <ChevronRight className="w-3 h-3" />
      ...
      <article className="max-w-4xl mx-auto px-4 py-12">
        <Link href="/presse" className="flex items-center gap-2 text-text-gray hover:text-primary text-sm mb-8">
          <ArrowLeft className="w-4 h-4" /> Retour à la presse
        </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            <span className="px-3 py-1 bg-rose-100 text-rose-700 rounded-full text-xs font-medium flex items-center gap-1">
              <Newspaper className="w-3 h-3" /> Presse
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6 text-sm text-text-gray mb-6">
            ...
            <span>{new Date(resource.createdAt).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</span>
          </div>
