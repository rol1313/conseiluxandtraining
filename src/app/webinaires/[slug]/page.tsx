...
import { ArrowLeft, Calendar, Clock, User, Video, ChevronRight, ExternalLink } from "lucide-react";
...
export default function WebinaireDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/resources/${slug}`)
      .then((res) => { if (res.status === 404) { router.push("/webinaires"); return; } return res.json(); })
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-2 text-sm text-text-gray">
          ...
          <Link href="/webinaires" className="hover:text-primary">Webinaires</Link>
          <ChevronRight className="w-3 h-3" />
      ...
      <article className="max-w-4xl mx-auto px-4 py-12">
        <Link href="/webinaires" className="flex items-center gap-2 text-text-gray hover:text-primary text-sm mb-8">
          <ArrowLeft className="w-4 h-4" /> Retour aux webinaires
        </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            <span className="px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-medium flex items-center gap-1">
              <Video className="w-3 h-3" /> Webinaire
            </span>
          </div>
